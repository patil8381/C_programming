...
tmp=(struct student *)realloc(s,sizeof(struct student)*(number_of_students_added+1));
printf("realloc called\n");
if(tmp==NULL)
{
printf("memory not allocated, memory not available, press enter key to continue...........");
getchar();
__fpurge(stdin);
}
s=tmp;
